...
const readStacks = async () => {
	const stackSection = (await readFile('05-input.txt', 'utf-8')).split(
		/\r?\n\r?\n/,
	)[0];
	...
	const stacks = new Array(stackCount).fill([]).map((_, i) => {
		const stack = [];
		for (const lineNumber of Array(stackLines.length - 1)
			.fill(0)
			.map((_, i) => i)
			.reverse()) {
...
const readCommands = async () => {
	const commandLines = (await readFile('05-input.txt', 'utf-8'))
		.split(/\r?\n\r?\n/)[1]
		.split(/\r?\n/);
...
const main = async (crateMover) => {
	if (![9000, 9001].includes(crateMover)) {
		throw new Error(
			'Invalid crate mover. Only CrateMover 9000 and 9001 are available.',
		);
	}
