...
const mainFirst = async () => {
  ...
  console.log({ pairsContainingEachOthersCount: pairsContainingEachOthers.length });
};

const mainSecond = async () => {
  const pairsOverlapping = (await readPairs()).filter(pair => {
    const pairOneNumbers = new Array(pair[0][1] - pair[0][0] + 1).fill(0).map((_, i) => pair[0][0] + i);
    const pairTwoNumbers = new Array(pair[1][1] - pair[1][0] + 1).fill(0).map((_, i) => pair[1][0] + i);
    if (
      pairOneNumbers.some(number => pairTwoNumbers.includes(number))
    ) {
      return true;
    }
    return false;
