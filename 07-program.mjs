...
const getNodeSize = (node, { disableSizeCache } = {}) => {
  ...
  const size = node.children.reduce((size, child) => size + getNodeSize(child), 0);
  if (!disableSizeCache) {
    node.size = size;
  }
  return size;
...
const generateFileTree = async () => {
  ...
  for (const command of commands) {
    const normalizedPath = currentWorkingDirectory.path === '/' ? '/' : currentWorkingDirectory.path + '/';
    if (command.instruction === 'cd') {
      if (command.params[0].startsWith('/')) {
        ...
        changeCWD(currentWorkingDirectory.path.split('/').slice(0, -1).join('/') || '/');
      } else {
      ...
      const newItems = items.filter(item => {
        ...
        return !currentWorkingDirectory.node.children.find(c => c.name === item.name);
      });
...
const main = async () => {
  ...
  const directoriesLowerThan100000 = collectNodes(node =>
    node.type === 'd' && getNodeSize(node) <= 100000
  );

  console.log('directories with size <= 100000:');
  console.log(directoriesLowerThan100000.map(d => `- ${d.name} (Size: ${d.size})`).join('\n'));
  console.log('With a total of: ' + directoriesLowerThan100000.reduce((size, d) => size + d.size, 0));
  ...
  const nodeToDelete =
    collectNodes(node =>
      node.type === 'd' && getNodeSize(node) >= missingDiskSpace
    )
    .sort((a, b) => getNodeSize(a) - getNodeSize(b))
    .at(0);
  console.log(`should delete directory: ${nodeToDelete.name} at path ${nodeToDelete.path} (Size: ${nodeToDelete.size})`);
}
