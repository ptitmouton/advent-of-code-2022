...
const runRound = (state, { nervous }) => {
	...
	const kgt = state.originalNotes.reduce(
		(acc, monkey) => acc * monkey.test.argument,
		1,
	);

	for (
		let monkeyIndex = 0;
		monkeyIndex < state.originalNotes.length;
		monkeyIndex++
	) {
		const note = state.originalNotes[monkeyIndex];
		state.monkeys[monkeyIndex].items = state.monkeys
			.at(monkeyIndex)
			.items.filter((angerLevelBeginning) => {
				...
				const testResult =
					angerLevelAfterMonkeyPlays % note.test.argument === 0;
...
const main = async () => {
	const mostActiveMonkeys = getMostActiveMonkeys(
		await playTheMonkeyGame({ rounds: 20, nervous: false }),
	);
	console.log('Part 1:');
	console.log('-------');
	...
	console.log(
		'which gives you a level of monkey business of ',
		mostActiveMonkeys.reduce((acc, monkey) => acc * monkey.itemsInspected, 1),
	);
	console.log('');
	...
	console.log(
		'which gives you an incredible level of monkey business of ',
		mostActiveNervousMonkeys.reduce(
			(acc, monkey) => acc * monkey.itemsInspected,
			1,
		),
	);
	console.log('');
