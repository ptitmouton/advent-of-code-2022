...
const moveKnot = (tail, head) => {
	...
	if (getDistance(tail, head) === 3) {
		throw new Error(
			`Knots are too far apart: [${tail.join(':')}, ${head.join(
				':',
			)}] -> ${getDistance(tail, head)}`,
		);
	}
...
const createHistoryOfKnotMovements = (numberOfKnots, commands) => {
	...
	for (const command of commands) {
		for (let i = 0; i < command.distance; i++) {
			...
			// first move the head
			const newKnots = [
				...currentState.knots.slice(0, -1),
				moveHead(currentState.knots.at(-1), command.direction),
			];
...
const main = async () => {
	...
	const numberOfTailPositions = (history) =>
		history
			...
			.filter((val, i, arr) => arr.indexOf(val) === i).length;

	console.log('Part 1:');
	console.log('------');
	console.log(
		`Number of positions visited by Tail at least once: ${numberOfTailPositions(
			createHistoryOfKnotMovements(2, commands),
		)}`,
	);
	...
	console.log(
		`Number of positions visited by Tail at least once: ${numberOfTailPositions(
			createHistoryOfKnotMovements(10, commands),
		)}`,
	);
};
