...
const getVisibility = (grid, { x, y }) => {
  ...
  let visibleTop, visibleBottom, visibleLeft, visibleRight,
      visibleDistanceFromTop, visibleDistanceFromBottom, visibleDistanceFromLeft, visibleDistanceFromRight;
  ...
  return {
    ...
    visible: (visibleTop ?? true) || (visibleBottom ?? true) || (visibleLeft ?? true) || (visibleRight ?? true),
    visibleDistanceFromTop,
    visibleDistanceFromBottom,
    visibleDistanceFromLeft,
    visibleDistanceFromRight,
    scenicScore: (visibleDistanceFromTop * visibleDistanceFromBottom * visibleDistanceFromLeft * visibleDistanceFromRight)
  };
...
const main = async () => {
  ...
  console.log('Part 1:');
  console.log('------');
  console.log(`There are ${visibleTrees.length} trees visible from the outside.`);
  ...
  const bestTree =
    getAllTrees(grid)
      ...
      .at(0);
  console.log(`The tree with the heighest scenig score is the tree at [${bestTree.x}, ${bestTree.y}]. It's height is ${bestTree.height} and is has an astounding scenic score of ${bestTree.scenicScore}.`);
}
